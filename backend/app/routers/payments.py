...
def _ensure_payment_scope(order: models.Order, user: models.User) -> None:
    # полный доступ
    if user.role in {models.Role.admin, models.Role.staff}:
        return

    # менеджер и бухгалтер — только свой филиал
...
def add_payment(
    ...
    current_user: models.User = Depends(
        require_roles(
            models.Role.admin,
            models.Role.manager,
            models.Role.accountant,
            models.Role.staff,  # добавили
        )
    ),
):
    ...
    payment = models.Payment(
        ...
        method=payload.method if isinstance(
            payload.method, str) else payload.method.value,
    ...
    return {"ok": True, "paid_amount": float(order.paid_amount), "payment_state": order.payment_state.value}
