    ...
    *,
    ...
    if manager_id:
        if role == models.Role.staff and manager_id != user.id:
            raise HTTPException(
                status_code=403, detail="Manager filter is not allowed")
...
def dashboard_summary(
    ...
    completed_states = {models.OrderStatus.tayyor,
                        models.OrderStatus.topshirildi}
    today = date.today()

    orders_total = len(orders)
    orders_completed = sum(1 for o in orders if o.status in completed_states)
    orders_in_progress = sum(
        1 for o in orders if o.status not in completed_states)
    orders_overdue = sum(
        1
        for o in orders
...
def dashboard_timeline(
    ...
    if order_ids:
        ...
        for payment in pay_query.all():
            bucket = _bucket_for(
                datetime.combine(payment.paid_at, datetime.min.time()
    ...
    timeline = [
        schemas.DashboardTimelinePoint(bucket=key, orders=int(
            val["orders"]), payments=round(val["payments"], 2))
...
def dashboard_activity(
    ...
    if current_user.role != models.Role.admin:
        ...
        if current_user.branch_id:
            conditions.append(models.AuditLog.branch_id ==
...
def dashboard_filters(
    ...
    doc_types = sorted({(o.doc_type or "").strip()
                       for o in orders if o.doc_type})
