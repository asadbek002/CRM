    ...
    return (
        <div className={`fixed inset-0 z-50 ${open ? "" : "pointer-events-none"}`}>
            ...
            >
                ...
                <div className="px-6">
                    {KIND_ORDER.map((k) => (
                        <FilesRow key={k} title={KIND_LABEL[k]} files={filesByKind[k]} loading={loadingFiles} />
                    ))}
                </div>
                ...
                <div className="p-6 border-t">
                    ...
                    <div className="flex gap-2 mb-4">
                        ...
                        />
                        <button onClick={submitComment} className="px-4 py-2 rounded-md bg-black text-white hover:opacity-90">
                            Yuborish
                        </button>
                    </div>
                        ...
                        {comments.map((c) => (
                            <li key={c.id} className="border rounded-md p-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-xs text-gray-400">{dayjs(c.created_at).format("YYYY-MM-DD HH:mm")}</div>
                                </div>
                                <div className="mt-1">{c.text}</div>
                                <button onClick={() => removeComment(c)} className="text-xs text-red-500 mt-2">
                                    O‘chirish
                                </button>
                            </li>
                        ))}
                        {comments.length === 0 && <div className="text-sm text-gray-500">Hozircha izoh yo‘q.</div>}
                    </ul>
                </div>
            </div>
        </div>
                ...
                <>
                    <div className="flex flex-wrap items-center gap-2">
                        {head.map((f) => (
                            <a
                                key={f.id}
                                href={`${baseURL}/attachments/${f.id}/download`}
                                className="file-chip"
                                title={f.original_name}
                            >
                                <span className="file-chip__icon">📎</span>
                                <span className="file-chip__name">{f.original_name}</span>
                                <span className="file-chip__size">{(f.size / 1024).toFixed(0)} KB</span>
                            </a>
                        ))}
                        ...
                        <div className="mt-3 flex flex-col gap-1">
                            {rest.map((f) => (
                                ...
                                >
                                    ...
                                    <span className="file-row__size">{(f.size / 1024).toFixed(0)} KB</span>
                                </a>
                            ))}
                            <button
